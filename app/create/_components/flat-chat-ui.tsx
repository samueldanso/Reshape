	...
	const { messages, sendMessage, status } = useChat({
		...
		onFinish: (message) => {
			...
			if (message.message.role === 'assistant' && message.message.parts) {
				console.log('Checking message parts for NFT data...')

				for (const part of message.message.parts) {
					console.log('Checking part:', part.type, part)

					if (part.type.startsWith('tool-')) {
						const toolPart = part as any
						console.log('Tool part found:', toolPart)
						...
						if (
							...
							console.log('NFT tool result found:', toolPart.output)
							try {
								const parsed = JSON.parse(toolPart.output)
								console.log('Parsed NFT data:', parsed)
								...
								if (
									...
									console.log('Setting pending transaction:', parsed)
									setPendingTransaction(parsed as PrepareMintSVGNFTData)
									toast.success(
										'🎉 NFT ready to mint! Check your wallet for approval.'
									)
									break
								}
							} catch (error) {
